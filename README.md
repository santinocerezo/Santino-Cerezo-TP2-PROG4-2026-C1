...
Spanegram es una red social desarrollada en dos proyectos separados (frontend Angular + backend NestJS) con base de datos MongoDB. Permite registrarse, iniciar sesión con **JWT**, publicar, dar "me gusta", **comentar**, **editar el perfil** y gestionar la sesión. En próximos sprints se suma el panel de administración.
...
- **Frontend:** Angular 22 (standalone components, signals, Reactive Forms), TypeScript, CSS (tema oscuro).
- **Backend:** NestJS 11, TypeScript, Mongoose.
- **Base de datos:** MongoDB Atlas.
- **Seguridad:** contraseñas encriptadas con **bcrypt** y autenticación con **JWT** (token con vencimiento de 15 min y una sola sesión activa por cuenta).
...
- **Frontend:**
  - **Página de detalle** de una publicación: se ve en grande junto con sus **comentarios** (ordenados, los más nuevos primero), con botón **"Cargar más"** que sigue trayendo sin perder los anteriores y un contador "Mostrando X de Y comentarios".
...
- **Editar perfil** (`PATCH /auth/perfil`): cambiar foto, descripción, nombre, apellido, fecha de nacimiento y correo; el nombre de usuario solo se puede cambiar una vez cada 15 días. Correo y usuario validan unicidad.
- **Una sola sesión activa por cuenta:** al iniciar sesión en otro dispositivo, la sesión anterior se cierra (401).
- **Validación de la fecha de nacimiento:** la edad debe estar entre 13 y 120 años (en registro y al editar el perfil).
- **Manejo de errores:** los errores de validación devuelven 400 con un mensaje claro en español (antes algunos casos devolvían 500).
- **Rediseño visual** a modo oscuro y marca **Spanegram** con favicon propio.
...
---
Trabajo Práctico desarrollado para Programación IV — 2026.
